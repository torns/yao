    ...
    <div>
        <el-popover
            ...
            <div class="header d2-clearfix d2-mb-10" v-if="clearable">
                <el-button type="danger" icon="el-icon-delete" size="mini" class="d2-fr" @click="selectIcon()">清空
                </el-button>
            </div>
            <el-input
                    v-model="searchText"
            ...
            <el-collapse v-if="!searchMode" class="group" v-model="collapseActive">
                <el-collapse-item v-for="(item, index) in icon" :key="index" :title="item.title" :name="index"
                    ...
                    <el-row class="class-row">
                        <el-col class="class-col" v-for="(iconName, iconIndex) in item.icon" :key="iconIndex" :span="4"
