<template>
    <div>
        <div class="search-container">
            <el-input v-if="sys_log_index_select" placeholder="请输入模块名称" v-model="table.query.moduleName"
                      style="width:200px" @keyup.enter.native="getData" clearable/>
            <el-select v-if="sys_log_index_select" v-model="table.query.status" placeholder="请选择状态">
                <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
            </el-select>
            <el-button v-if="sys_log_index_select" class="search-btn" type="primary" icon="el-icon-search"
                       @click="getData">查询
            </el-button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.getData()
            this.sys_log_index_select = this.permissions['/syslog/log/index:select']
        },
        computed: {
