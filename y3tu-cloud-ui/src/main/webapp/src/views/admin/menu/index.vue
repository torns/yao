        ...
        <tree-table :data="data" :expand-all="expandAll" :columns="columns" border>
          ...
          <span>
            ...
            <el-table-column label="操作" align="center" width="200"
                             v-if="sys_resource_add || sys_resource_update || sys_resource_delete">
                <template slot-scope="scope">
                    <el-button type="primary" size="mini" class="mb5" @click="openDialog(scope.row.id)"
                               icon="el-icon-plus" v-if="scope.row.type == 0 && sys_resource_add"></el-button>
                    <el-button type="success" size="mini" class="ml10" @click="openEditDialog(scope.row.id)"
                               icon="el-icon-edit" v-if="sys_resource_update"></el-button>
                    <el-button type="danger" size="mini" @click="deleteHandle(scope.row.id)" icon="el-icon-delete"
                               v-if="sys_resource_delete"></el-button>
                </template>
            </el-table-column>
        </tree-table>

        <!-- 添加菜单信息 -->
        <el-dialog :title="dialog.title" :visible.sync="dialog.show" :before-close="closeHandle" width="600px"
                   :close-on-click-modal="false">
            <div class="dialog-container">
                <el-alert class="alert" title="为方便操作，添加时[权限标识/组件路径]会自动继承父级的资源属性" type="info" center
                          show-icon></el-alert>
