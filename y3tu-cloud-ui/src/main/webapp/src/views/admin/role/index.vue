<template>
    <div>
        <div class="search-container">
            <el-input placeholder="请输入角色名称" v-model="table.query.roleName" style="width:200px"
                      @keyup.enter.native="getData" clearable/>
            <el-input placeholder="请输入角色代码" v-model="table.query.roleCode" style="width:200px"
                      @keyup.enter.native="getData" clearable/>
            <el-button class="search-btn" type="primary" icon="el-icon-search" @click="getData" v-if="sys_role_select">
                查询
            </el-button>
            <el-button class="search-btn" type="primary" icon="el-icon-plus" @click="openDialogHandle"
                       v-if="sys_role_add">添加
            </el-button>
            <el-button class="search-btn" :autofocus="true" icon="el-icon-refresh" @click="refreshHandle">刷新</el-button>
        </div>
        <el-table v-loading="table.loading" :data="table.data" :default-sort="{prop : 'roleName', prop: 'roleCode'}"
            ...
            <el-table-column align="center" label="操作">
                <template slot-scope="scope">
                    <el-button size="mini" class=" mb5" @click="openDialog(scope.row.roleId)"
                               icon="el-icon-search"></el-button>
                    <el-button type="success" size="mini" class="ml10" @click="editDialogHandle(scope.row.roleId)"
                               icon="el-icon-edit" v-if="sys_role_update"></el-button>
                    <el-button type="danger" size="mini" @click="deleteHandle(scope.row.roleId)" icon="el-icon-delete"
                               v-if="sys_role_delete"></el-button>
                </template>
            </el-table-column>
        </el-table>
        <div v-show="!table.loading" class="footer">
            <el-pagination @size-change="sizeChangeHandle" @current-change="currentChangeHandle"
                           :current-page.sync="table.query.current" :page-size="table.query.size" :total="table.total"
        ...
        <el-dialog :title="dialog.title" :visible.sync="dialog.show" width="600px" :close-on-click-modal="false">
            <el-form ref="roleForm" :model="dialog.data" :rules="dialog.rules" label-width="80px"
                ...
                <el-form-item label="权限集合">
                    <el-tree ref="permissionTree" :data="permissionTree" show-checkbox node-key="id" :props="props"
                             default-expand-all :default-checked-keys="checkedIds">
                    </el-tree>
                </el-form-item>
            </el-form>
            <span slot="footer">
